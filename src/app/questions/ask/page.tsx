...
// Is container ko sahi kiya taaki content upar na chipke
const LabelInputContainer = ({
  ...
  return (
    <div
      className={cn(
        "relative flex w-full flex-col space-y-2 rounded-xl border border-white/20 bg-slate-950 p-6 shadow-sm",
        className,
      )}
    >
      {children}
...
const QuestionForm = ({ question }: { question?: Models.Document }) => {
  ...
  const [formData, setFormData] = React.useState<any>({
    title: String((question as any)?.title || ""),
    content: String((question as any)?.content || ""),
    authorId: user?.$id,
    tags: new Set(((question as any)?.tags || []) as string[]),
  ...
  const update = async () => {
    ...
    const attachmentId = await (async () => {
      if (!formData.attachment)
        return (question as any)?.attachmentId as string;

      await storage.deleteFile(
        questionAttachmentBucket,
        (question as any).attachmentId,
      );
  ...
  return (
    /* pt-32 yahan sabse important hai jo navbar se niche layega */
    <div className="relative min-h-screen w-full bg-slate-950 px-4 pb-20 pt-32">
      <Meteors number={30} />

      <form className="mx-auto max-w-4xl space-y-8" onSubmit={submit}>
        <h1 className="text-2xl font-bold text-white mb-6">
          {question ? "Edit your question" : "Ask a public question"}
        </h1>
